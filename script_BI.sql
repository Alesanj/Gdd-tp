...
IF OBJECT_ID('obtenerRangoEtario', 'FN') IS NOT NULL
DROP FUNCTION obtenerRangoEtario;
GO
CREATE FUNCTION obtenerRangoEtario(@fechaNac date) RETURNS int 
AS
BEGIN
...
AS
	...
	JOIN BI_MODEL.RANGO_ETARIO t_etario ON dbo.obtenerRangoEtario(t_repartidor.REPARTIDOR_FECHA_NAC) = t_etario.RANGO_ETARIO_NRO
	JOIN BI_MODEL.TIEMPO t_tiempo ON t_tiempo.MES = MONTH(t_envio_mensajeria.ENVIO_MENSAJERIA_FECHA_ENTREGA)
...
/*
select * from BI_MODEL.HECHO_ENVIO
drop table BI_MODEL.HECHO_ENVIO
exec BI_MIGRAR_HECHO_ENVIOS



select SUM(HECHO_ENVIO_VALOR_PROMEDIO)/COUNT(HECHO_ENVIO_VALOR_PROMEDIO) from BI_MODEL.HECHO_ENVIO

SELECT SUM(MENSAJERIA_VALOR_ASEGURADO)/COUNT(MENSAJERIA_VALOR_ASEGURADO) from NEW_MODEL.MENSAJERIA 

SELECT SUM(PEDIDO_PRECIO_ENVIO)/COUNT(PEDIDO_PRECIO_ENVIO) from NEW_MODEL.PEDIDO 
*/